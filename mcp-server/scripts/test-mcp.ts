...
async function main() {
  ...
  await check("tools/list returns ask_rehman + list_nodes", async () => {
    ...
    if (!names.includes("ask_rehman")) throw new Error(`Missing ask_rehman. Got: ${names.join(", ")}`);
    if (!names.includes("list_nodes")) throw new Error(`Missing list_nodes. Got: ${names.join(", ")}`);
  });
  ...
  await check("tools/call list_nodes returns node list", async () => {
    ...
    if (json?.error) throw new Error(`RPC error: ${JSON.stringify(json.error)}`);
    const content = (json?.result as { content?: Array<{ text?: string }> } | undefined)?.content ?? [];
    if (content.length === 0) throw new Error("Empty content");
    const text = content[0]?.text ?? "";
    if (!text.includes("[")) throw new Error(`Unexpected content: ${text.slice(0, 80)}`);
  });
    ...
    await check("tools/call ask_rehman returns cited text", async () => {
      ...
      const { json } = await mcpPost(
        baseUrl,
        {
          ...
          id: 4,
          method: "tools/call",
          params: { name: "ask_rehman", arguments: { query: "What is Kalrav.AI?" } },
        },
        sessionId
      );
      if (json?.error) throw new Error(`RPC error: ${JSON.stringify(json.error)}`);
