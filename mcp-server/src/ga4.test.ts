...
import { classifyMcpClient, sendGa4Event, trackMcpConnected } from './ga4.js';
...
describe('sendGa4Event', () => {
  ...
  it('POSTs to the GA4 Measurement Protocol when both env vars are set', async () => {
    ...
    await sendGa4Event({
      clientId: 'session-1',
      events: [
        { name: 'agent_mcp_connected', params: { client_bucket: 'curl' } },
      ],
      fetchImpl,
    });
...
describe('trackMcpConnected', () => {
  ...
  it('never sends the raw UA, only the bucket', async () => {
    ...
    await trackMcpConnected(
      'sess',
      'Claude-Desktop/2.1.4 (macOS 14.1)',
      fetchImpl,
    );
